...
export class DividendCalculator {
  ...
  calculateDividendProjection(params: InvestmentParameters): SimulationResults {
    ...
    const totalAllocation = params.allocations.reduce(
    ...
    for (let month = 1; month <= totalMonths; month++) {
      ...
      const monthlyGrowthRate =
        this.calculateWeightedGrowthRate(params.allocations) / 12;
   ...
   */
  private calculateMonthlyDividend(
   ...
   */
  private calculateWeightedGrowthRate(
    allocations: PortfolioAllocation[],
  ): number {
    ...
    allocations.forEach((allocation) => {
      ...
      if (stock) {
        weightedGrowthRate +=
          (stock.annualGrowthRate * allocation.percentage) / 100;
      }
    });
