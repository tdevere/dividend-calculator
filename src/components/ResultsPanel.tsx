...
export default function ResultsPanel({
  ...
  const weightedYield = calculator.calculateWeightedDividendYield(
    params.allocations,
  );
  const totalReturn =
    (results.finalPortfolioValue / results.totalInvestmentMade - 1) * 100;
  const annualizedReturn =
    Math.pow(
      results.finalPortfolioValue / results.totalInvestmentMade,
      1 / params.investmentDuration,
    ) - 1;
  const dividendYield =
    (results.totalDividendsReceived / results.totalInvestmentMade) * 100;
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center p-3 bg-slate-700/30 rounded-lg">
                    ...
                    <span className="font-bold text-cyan-400">
                      {formatCurrency(results.totalInvestmentMade)}
                    </span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-slate-700/30 rounded-lg">
                  ...
                  <div className="flex justify-between items-center p-3 bg-slate-700/30 rounded-lg">
                    ...
                    <span className="font-bold text-purple-400">
                      {formatCurrency(results.totalDividendsReceived)}
                    </span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-slate-700/30 rounded-lg">
                    <span className="text-slate-300">Total Return</span>
                    <span className="font-bold text-pink-400">
                      {formatPercentage(totalReturn, 1)}
                    </span>
                  </div>
                </div>
              </div>
                      ...
                      >
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className="text-cyan-400 font-bold">
                            {formatPercentage(allocation.percentage, 0)}
                          </span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span className="text-slate-400">
                            Yield: {formatPercentage(stock.dividendYield, 1)}
                          </span>
                          <span className="text-slate-400">
                            Growth:{" "}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {monthlyIncomeGoals.map((goal, index) => {
                ...
                const currentMonthlyIncome =
                  results.monthlyResults[results.monthlyResults.length - 1]
                  ...
                  >
                    ...
                    <div className="text-sm text-slate-400">
                      {achieved ? (
                        ...
                        <span>
                          Achievable in {Math.ceil(monthsToGoal / 12)} years
                        </span>
                      ) : (
